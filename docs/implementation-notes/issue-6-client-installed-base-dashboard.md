...
- Added an Expo (React Native) renderer (`src/App.tsx`) seeded from the synthetic fixtures: renders all four view states via demo controls, plus the ready view with Modality/brand/model filter chips (selectors derived from the seeded base) and the cross-Client/Site aggregation panel. Cross-platform web + mobile per the QVAC seam decision.
