...
The field collaborator captures observations while inside a hospital, frequently without stable connectivity, and the captured content is sensitive client data. The challenge mandates that capture and extraction work without the internet and never send content to an external service; QVAC enables on-device and peer-to-peer inference. Solutions that send inference to a cloud API are disqualified, and a future reader would otherwise assume the easier path of calling a cloud API.
...
- Positive: the app works offline in the field; sensitive client data never leaves the device; the solution satisfies the challenge constraint.
