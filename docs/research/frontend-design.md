...
Research for the FieldSite MVP: a Collaborator captures a Field note during a Visit, reviews structured Observations, and explores the Installed base by Client and Site. The interface must work on web and mobile, with local-first loading and offline states.
