...
export function DesktopSidebar({ activeSection, onNavigate }: { readonly activeSection: AppSection; readonly onNavigate: (section: AppSection) => void }) {
  ...
  return <View style={{ width: 216, backgroundColor: colors.surface, borderRightWidth: 1, borderRightColor: colors.border, paddingHorizontal: spacing.lg, paddingTop: spacing.xl, paddingBottom: spacing.lg, justifyContent: 'space-between' }}><View><View style={{ marginBottom: spacing.xxxl }}><FieldSightLockup markSize={40} /></View>{items.map(([section, Icon, label]) => <TouchableOpacity key={section} accessibilityRole="button" accessibilityLabel={label} onPress={() => onNavigate(section)} style={{ minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingHorizontal: spacing.md, marginBottom: spacing.xs, borderRadius: radius.sm, backgroundColor: activeSection === section ? colors.primarySoft : 'transparent', borderLeftWidth: activeSection === section ? 3 : 0, borderLeftColor: colors.primary }}><Icon size={18} color={activeSection === section ? colors.primary : colors.textSecondary} /><Text style={{ color: activeSection === section ? colors.primary : colors.textSecondary, fontSize: typography.sizes.sm, fontWeight: activeSection === section ? typography.weights.semibold : typography.weights.medium }}>{label}</Text></TouchableOpacity>)}<TouchableOpacity accessibilityRole="button" accessibilityLabel="Settings" style={{ minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingHorizontal: spacing.md }}><Settings size={18} color={colors.textSecondary} /><Text style={{ color: colors.textSecondary, fontSize: typography.sizes.sm }}>Settings</Text></TouchableOpacity></View><Text style={{ color: colors.textMuted, fontSize: typography.sizes.xs }}>LOCAL-FIRST · MVP</Text></View>;
}
