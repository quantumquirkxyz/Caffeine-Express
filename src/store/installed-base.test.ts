...
describe('InstalledBase', () => {
  it('resolves complete observations and excludes incomplete keys', async () => {
  ...
  it('reconciles matching reports without inventing values', async () => {
    ...
    expect(base.find({ model: 'NM-MR 700' })[0]?.state).toBe('Confirmed');
    expect(base.find({ model: 'NM-MR 700' })[0]?.use).toBeNull();
  });

  it('aggregates quantity across Sites and Clients', async () => {
    const store = new MemoryObservationStore();
    const observations = await loadSyntheticFixtures(store);
    const base = new InstalledBase();
    observations.forEach((observation) => base.update(observation));
    expect(base.aggregate({ modality: 'MRI' }).quantity).toBe(9);
    expect(base.aggregate({ brand: 'BluePeak Medical' }).quantity).toBe(4);
